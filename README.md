...
- **审核员（reviewer）**：在审核中心查看全站稿件，在线预览视频，通过 / 驳回（必填原因），审核通过后手动上传B站并回填BV号标记发布。
...
- 接口冒烟测试：`node scripts/smoke-test.mjs`（主流程）、`node scripts/smoke-autopublish.mjs`（自动投稿链路）、`node scripts/smoke-bili.mjs`（B站绑定与保活链路，需联网），后两者需 mock 配置启动后端（见各脚本头部注释）。
