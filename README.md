...
协会内部自用的B站视频上传审核平台，前后端分离架构。成员上传视频 → 审核员审核 → 通过后由运营发布到协会B站账号并回填BV号归档。
...
首次启动后端会自动建库，并创建默认管理员：**admin / admin123456**（请登录后尽快修改密码）。

## 角色与流程

- **成员（member）**：上传视频、编辑草稿、提交审核、撤回、查看驳回原因后重新提交。
- **审核员（reviewer）**：在审核中心查看全站稿件，在线预览视频，通过 / 驳回（必填原因），审核通过后手动上传B站并回填BV号标记发布；可用协会账号发布B站动态。
- **管理员（admin）**：审核员全部权限 + 成员管理（建号、改角色、禁用、重置密码）。
...
| `INVITE_CODE` | 空（开放注册） | 设置后注册必须填写邀请码 |
...
绑定账号后，审核员和管理员可在「B站动态」页面以协会账号发布动态（公告、活动宣传等）：正文最多 1000 字，配图最多 9 张（jpg/png/gif，单张 ≤20MB，先传B站图床再发布）。发布记录（发布人、内容、动态链接）保存在平台内可追溯。走B站网页端接口，与投稿一样属于非官方渠道，请控制发布频率。
...
| GET | `/api/review/videos` | 全站稿件（审核员） |
